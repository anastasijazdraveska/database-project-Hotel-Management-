...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            RAISERROR(
                'Actual check-in date does not exist.',
                16,
                1
            );
        ...
        -------------------------------------------------
        -- If no checkout date is supplied,
        ...
        BEGIN
            RAISERROR(
                'Actual check-out date cannot be before actual check-in.',
                16,
                1
            );
        ...
        BEGIN
            RAISERROR(
                'Actual check-out date cannot be before the scheduled check-in date.',
                16,
                1
            );
        ...
        SELECT
            @CleaningStatusID = RoomStatusID
        FROM RoomStatus
        WHERE StatusName = 'Cleaning';


        IF @CleaningStatusID IS NULL
        BEGIN
            RAISERROR(
                'Room status Cleaning does not exist.',
                16,
                1
            );
